...
export const CNCTable: React.FC<CNCTableProps> = ({
  data,
  loading = false,
  title
}) => {
    ...
    return (
      <Card>
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">No CNC Data Available</h3>
          <p className="text-gray-500">No CNC data available for the selected period</p>
        </div>
      </Card>
    );
  }
    ...
    <Card>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200">
              <th className="text-center py-3 px-4 font-medium text-gray-700">CNC Machine</th>
              ...
              <th className="text-center py-3 px-4 font-medium text-gray-700">Inactive Days</th>
            </tr>
          </thead>
          <tbody>
            {sortedData.map((item, index) => {
              ...
              const inactivePercentage = item.totalDaysInRange > 0 ? (item.inactiveDays / item.totalDaysInRange) * 100 : 0;
              const isInactive = inactivePercentage > 50; // Más del 50% de días inactivos
                ...
                >
                  ...
                  <td className="py-4 px-4">
                    <div className="flex items-center justify-center">
                      ...
                      `}>
                        {item.inactiveDays} days ({inactivePercentage.toFixed(1)}%)
                      </span>
                    </div>
                  </td>
                </tr>
      ...
      <div className="mt-6 pt-4 border-t border-gray-200">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-gray-900">
              {data.reduce((sum, item) => sum + item.inactiveDays, 0)}
            </div>
            <div className="text-sm text-gray-600">Total Inactive Days</div>
          </div>
        </div>
      </div>
    </Card>
